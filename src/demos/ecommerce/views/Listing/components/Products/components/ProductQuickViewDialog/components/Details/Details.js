...
const Details = ({
  ...
  const changeSize = useCallback((item) => () => setSize(item), []);
  const changeColor = useCallback((item) => () => setColor(item), []);
  const changeQuantity = useCallback((modificator) => () => {
    if (modificator < 0) {
      setQuantity(quantity - 1 >= 1 ? quantity - 1 : 1);
    } else {
      setQuantity(quantity + 1 <= quantityLimit ? quantity + 1 : quantityLimit);
    }
  }, [quantity]);
