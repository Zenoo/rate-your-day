...
const Places = () => {
  ...
  const lightboxPrev = useCallback(() => setCurrentImage((currentImage + photos.length - 1) % photos.length), [currentImage, photos.length]);
  const lightboxNext = useCallback(() => setCurrentImage((currentImage + 1) % photos.length), [currentImage, photos.length]);
